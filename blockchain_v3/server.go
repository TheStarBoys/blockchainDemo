...
var nodeAddress string // 监听地址
var miningAddress string
var knownNodes = []string{"localhost:3000"}
var blocksInTransit = [][]byte{}
var mempool = make(map[string]Transaction)
var minTxNum = 2 // 开始挖矿的最小交易数
...
func sendData(dstAddr string, data []byte) {
	...
	if err != nil {
		...
		for _, node := range knownNodes {
			if node != dstAddr {
				updateAddress = append(updateAddress, node)
			}
		}

		knownNodes = updateAddress
	}
	defer conn.Close()
...
func handleTx(request []byte, bc *Blockchain) {
	...
	tx := DeserializeTransation(txData)
	...
	if nodeAddress == knownNodes[0] {
		...
		//MineTransactions:
			...
			coinbase := NewCoinbaseTX(miningAddress, "")
